...
// Paths for reuse
const exportPath = './app/dist/**/*';
const srcPath = (file, watch = false) => {
  if (file === 'scss' && watch === false) return './app/src/scss/styles.scss';
  if (file === 'scss' && watch === true) return './app/src/scss/**/*.scss';
  if (file === 'js' && watch === false) return entryArray;
  if (file === 'js' && watch === true) return './app/src/js/**/*.js';
  if (file === 'html') return './app/src/**/*.html';
  if (file === 'img') return './app/src/img/**/*.{png,jpeg,jpg,svg,gif}';
  console.error('Unsupported file type entered into Gulp Task Runner for Source Path');
};
const distPath = (file, serve = false) => {
  if (['css', 'js', 'img'].includes(file)) return `./app/dist/${file}`;
  if (file === 'html' && serve === false) return './app/dist/**/*.html';
  if (file === 'html' && serve === true) return './app/dist';
...
const cleanExport = (mode) => () => {
  return ['development', 'production'].includes(mode) ? del(['./app.zip']) : undefined;
};
